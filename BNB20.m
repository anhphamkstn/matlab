...
Z=[]; X=[]; t=0; c=0; fail=0;
if nargin<2, errmsg='BNB needs at least 2 input arguments.'; return; 
end
if isempty(fun), errmsg='No fun found.'; return;
% elseif ~ischar(fun), errmsg='fun must be a string.'; return; 
end
   ...
   if isnumeric(xstat) && isreal(xstat) && all(size(xstat)<=size(x0))
      if all(xstat==round(xstat) & 0<=xstat & xstat<=2)
         xstatus(1:size(xstat))=xstat;
          ...
          errmsg='xstatus must consist of the integers 0,1 en 2.'; 
          return;
      end
   else
       errmsg='xstatus must be a real column vector the same size as x0.'; return; 
   end
end
xlb=zeros(size(x0));
xlb(xstatus==0)=-inf;
if nargin>3 && ~isempty(xl)
   if isnumeric(xl) && isreal(xl) && all(size(xl)<=size(x0))
      xlb(1:size(xl,1))=xl;
       ...
       errmsg='xlb must be a real column vector the same size as x0.'; return; 
   end
end
...
if nargin>4 && ~isempty(xu)
   if isnumeric(xu) && isreal(xu) && all(size(xu)<=size(x0))
      xub(1:size(xu,1))=xu;
       ...
       errmsg='xub must be a real column vector the same size as x0.'; return;
   end
end
...
if nargin>5 && ~isempty(a)
   if isnumeric(a) && isreal(a) && size(a,2)==size(x0,1), A=a;
       ...
       errmsg='Matrix A not correct.'; return; 
   end
end
B=[];
if nargin>6 && ~isempty(b)
   if isnumeric(b) && isreal(b) && all(size(b)==[size(A,1) 1]), B=b;
       ...
       errmsg='Column vector B not correct.'; return; 
   end
end
...
if nargin>7 && ~isempty(aeq)
   if isnumeric(aeq) && isreal(aeq) && size(aeq,2)==size(x0,1), Aeq=aeq;
       ...
       errmsg='Matrix Aeq not correct.'; return; 
   end
end
Beq=[];
if nargin>8 && ~isempty(beq)
   if isnumeric(beq) && isreal(beq) && all(size(beq)==[size(Aeq,1) 1]), Beq=beq;
       ...
       errmsg='Column vector Beq not correct.'; return; 
   end
end
...
if nargin>9 && ~isempty(nonlc)
   if ischar(nonlc), nonlcon=nonlc; 
       ...
       errmsg='fun must be a string.'; return; 
   end
end
settings = [0 0];
if nargin>10 && ~isempty(setts)
   if isnumeric(setts) && isreal(setts) && all(size(setts)<=size(settings))
      settings(setts~=0)=setts(setts~=0);
       ...
       errmsg='settings should be a row vector of length 1 or 2.'; return; 
   end
end
maxSQPiter=1000;
      ...
      if isfield(opts,'MaxSQPIter')
         if isnumeric(opts.MaxSQPIter) && isreal(opts.MaxSQPIter) && ...
            ...
            opts=rmfield(opts,'MaxSQPIter');
             ...
             errmsg='options.maxSQPiter must be an integer >0.'; return; 
         end
      end
      options=optimset(options,opts);
   else
       errmsg='options must be a structure.'; return;
   end
end
evalreturn=0;
   ...
   for i=1:size(Aeq,1)
      if Beq(i)==1 && all(Aeq(i,:)==0 | Aeq(i,:)==1)
         ...
         if all(xstatus(J)~=0 & xchoice(J)==0 & xlb(J)==0 & xub(J)==1)
            if all(xstatus(J)~=2) || all(x0(J(xstatus(J)==2))==0)
               j=j+1;
               xchoice(J)=j;
               if sum(x0(J))==0, errmsg='x0 not correct.'; return; 
               end
            end
         end
      end
...
while stacksize>0
   ...
   fprintf('*** searched %3d %% of three\n',percdone)
   fprintf('*** Z    : %12.4e\n',z_incumbent)
   fprintf('*** t    : %12.1f secs\n',t)
   fprintf('*** c    : %12d cycles\n',c-1)
   fprintf('*** fail : %12d cycles\n',fail)
